...
//! date-format DRIFT detectors. Ported faithfully from redpash-rust-pwa (the
//! calibration is the moat). Sentinel checks route through `crate::sentinels`.
//! FR-first: day-first/2-digit-year date shapes, FR+EN bool words.
...
mod tests {
    ...
    fn classify_cell_kinds() {
        ...
        assert_eq!(classify_cell("inconnu"), CellKind::Empty); // FR sentinel
    }
