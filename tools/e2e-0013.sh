...
# GET with status + body captured separately.
...
http_options() { # $1=path → sets HTTP_CODE, HTTP_BODY
  HTTP_CODE="$(curl -sS -X OPTIONS -o /tmp/e2e0013.body -w '%{http_code}' "$BASE$1")" || true
  HTTP_BODY="$(cat /tmp/e2e0013.body 2>/dev/null || true)"
}
...
# ── AC1: GET /api/types/file carries context_view ────────────────────────────
http_get "/api/types/file"
...
# ── AC1: GET /api/types (catalog) carries context_view per entry ──────────────
http_get "/api/types"
if [ "$HTTP_CODE" = "200" ] && printf '%s' "$HTTP_BODY" | grep -q '"context_view"'; then
...
# ── AC1: OPTIONS /api/objects/file options_body carries context_view (parity) ─
http_options "/api/objects/file"
if printf '%s' "$HTTP_BODY" | grep -q '"context_view"'; then
  ok 'AC1  OPTIONS /api/objects/file → options_body has "context_view" (parity)'
else
  bad "AC1  OPTIONS /api/objects/file missing context_view (code=$HTTP_CODE)"
fi
...
printf '\nCASE 0013 E2E: %d passed, %d failed\n' "$pass" "$fail"
...
#
#   0. Seed + serve:   tools/seed-demo.sh ; start the binary with REDPASH_DEV_LOGIN
#      analog for numu (REDPASH_* is RedPash — for numu use the dev-login env the
#      repo defines) and NUMU_WEB_DIR pointing at numu/web.
