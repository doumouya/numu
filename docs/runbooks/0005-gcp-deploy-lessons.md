...
| 7 | **THE BACKUP THAT NEVER WAS** — `numu-pg-dump.service` failed on every run; the bucket was EMPTY when the pg18 restore drill needed it (the "restore" silently restored nothing: a fresh DB, a re-minted member actor, missing articles). TWO layers: (a) a bash syntax error baked into the unit; (b) after fixing (a), `AccessDeniedException: 403 … does not have storage.objects.list access` on the serial console — Cloud Logging was blind because the Ops Agent wasn't shipping either | (a) the inline `ExecStart=… \$\$(date +%%F) …` crossed FOUR escaping layers (local double-quotes → remote unquoted heredoc → systemd → bash); the heredoc expanded `$$` to its own PID. (b) the VM SA is **create-only on the bucket by design**, and `gsutil cp -` (streaming) demands bucket LIST before uploading — two least-privilege systems meeting, again | the wrapper is built LOCALLY and shipped **base64** (escape-proof by construction) and uploads via a streaming `curl` PUT to the GCS XML API — needs ONLY `objects.create`, so the SA stays narrow (a backup bucket the source host can neither list nor delete is ransomware-resistant); `vm-postgres.sh` RUNS the dump once at provision and fails loud, and verifies the Ops Agent is ACTIVE — a backup job that has never succeeded is not a backup job |
