...
| `NUMU_API_RATE_LIMIT` / `_WINDOW_SECS` | `1200` / `60` | per-client limit on the AUTHENTICATED data surface (`/api/objects`, `/api/search`, `/api/nacl`, relations, conversations) — a generous backstop so one valid session can't hammer the DB; `/healthz`/`/readyz` are never throttled (CAS_57309651) |
| `NUMU_MAX_BODY_BYTES` | `2097152` (2 MiB) | explicit request body-size cap (a tighter inner limit like the app-tier ingest's 64 KiB wins for that route). Security response headers (`X-Content-Type-Options`, `X-Frame-Options: DENY`, `CSP frame-ancestors 'none'`, `Referrer-Policy`, `Permissions-Policy`, HSTS on release) ship on every response — no env needed (CAS_57309651) |
