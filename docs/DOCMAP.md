...
| [`.claude/skills/http-contract-safety/`](../.claude/skills/http-contract-safety/SKILL.md) | How-to: keep the live **LAYERED** HTTP contract honest — the credentialed-CORS policy (explicit allowlist, never `*`, exact-origin echo, preflight-accuracy, keep-for-prod) + the contract-shadow discipline (`build_router` single source of truth + boot self-check + no-DB full-stack regression) — *applies* `HTTP.md` + ADR `0004` | `crates/api/src/{lib,cors}.rs` · `tests/options_routing.rs` · `tools/e2e-0013.sh` | **shipped** (CASE 0017) |
