...
| [`apps/DOCS.md`](apps/DOCS.md) | the Docs app: markdown documents as objects (the `document` type, 0025) + the reusable vanilla-TS editor core (react-md-editor as spec, not code) + the **draft-guard** discipline every console editor adopts (dirty guard · localStorage draft restore · honest save/412 state) | `web/src/apps/{docs,md-editor,md-ops,draft-guard}.ts` · `migrations/0025` · [`frontend/CONSOLE.md`](frontend/CONSOLE.md) | **live** (CAS_25dbaded) |
