...
| `shell/auth.ts` | the identity store (`auth.identity`, null in sim) · `fetchMe` (the gate decision, pure over the seam's `request()`) · `doLogout` (204 → full reload back through the gate — mounts have no teardown contract) |
