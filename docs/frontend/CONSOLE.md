...
| `console/settings.ts` | the Settings surface (opens expanded in the panel): account (+ Sign out when a session exists) · **Directory** (HTTP mode, CAS_e6695638/CAS_6b46bea7: create workspaces/users AND a **Manage** sub-list — inline rename + status via PATCH If-Match; NO delete, the backend has no cascade) · appearance (skin cards + density) · workspace · members & roles · apps & permissions · AI-connect · plan · notifications · security & sessions · data · support · danger |
